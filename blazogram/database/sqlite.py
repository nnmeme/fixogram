...
class SQLite3(Database):
    ...
    @classmethod
    async def connect(cls, database: str):
        connection = await connect(database)
        return cls(connection)

    async def start(self):
        query = 'CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL, first_name TEXT NOT NULL, last_name TEXT NOT NULL, username TEXT NOT NULL, is_bot BOOLEAN NOT NULL)'
